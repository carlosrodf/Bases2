...
class crReport1_summary extends crReport1 {
	...
	function Page_Init() {
		...
		// Setup placeholder
		$this->Establecimiento->PlaceHolder = $this->Establecimiento->FldCaption();
...
//		$item->Visible = TRUE;
	...
	function Page_Main() {
		...
		$this->Oficial->SelectionList = "";
		$this->Oficial->DefaultSelectionList = "";
		$this->Oficial->ValueList = "";
	...
	//		$rs->MoveFirst(); // NOTE: no need to move position
			if ($this->GrpCount == 1) {
				...
				$this->FirstRowData['Oficial'] = ewr_Conv($rs->fields('Oficial'),200);
			}
		} else { // Get next row
			$rs->MoveNext();
		}
		if (!$rs->EOF) {
			...
			$this->Oficial->setDbValue($rs->fields('Oficial'));
			$this->Val[1] = $this->Establecimiento->CurrentValue;
			$this->Val[2] = $this->Oficial->CurrentValue;
		} else {
			$this->Usuario->setDbValue("");
			$this->Nombre->setDbValue("");
			$this->Establecimiento->setDbValue("");
			$this->Oficial->setDbValue("");
		}
	}
	...
	function ProcessAjaxPopup() {
		...
		if (@$_GET["popup"] <> "") {
			...
			// Check popup name
			// Build distinct values for Oficial

			if ($popupname == 'Report1_Oficial') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
				$sSql = ewr_BuildReportSql($this->Oficial->SqlSelect, $this->getSqlWhere(), $this->getSqlGroupBy(), $this->getSqlHaving(), $this->Oficial->SqlOrderBy, $sFilter, "");
				$rswrk = $conn->Execute($sSql);
				while ($rswrk && !$rswrk->EOF) {
					$this->Oficial->setDbValue($rswrk->fields[0]);
					if (is_null($this->Oficial->CurrentValue)) {
						$bNullValue = TRUE;
					} elseif ($this->Oficial->CurrentValue == "") {
						$bEmptyValue = TRUE;
					} else {
						$this->Oficial->ViewValue = $this->Oficial->CurrentValue;
						ewr_SetupDistinctValues($this->Oficial->ValueList, $this->Oficial->CurrentValue, $this->Oficial->ViewValue, FALSE, $this->Oficial->FldDelimiter);
					}
					$rswrk->MoveNext();
				}
				if ($rswrk)
					$rswrk->Close();
				if ($bEmptyValue)
					ewr_SetupDistinctValues($this->Oficial->ValueList, EWR_EMPTY_VALUE, $ReportLanguage->Phrase("EmptyLabel"), FALSE);
				if ($bNullValue)
					ewr_SetupDistinctValues($this->Oficial->ValueList, EWR_NULL_VALUE, $ReportLanguage->Phrase("NullLabel"), FALSE);
				$fld = &$this->Oficial;
			}
	...
	function SetupPopup() {
		...
		// Get 'reset' command
			...
			if (strtolower($sCmd) == "reset") {
				$this->ClearSessionSelection('Oficial');
				$this->ResetPager();
			}
		}

		// Load selection criteria to array
		// Get Oficial selected values

		if (is_array(@$_SESSION["sel_Report1_Oficial"])) {
			$this->LoadSelectionFromSession('Oficial');
		} elseif (@$_SESSION["sel_Report1_Oficial"] == EWR_INIT_VALUE) { // Select all
			$this->Oficial->SelectionList = "";
		}
	}
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// Oficial
			$this->Oficial->ViewValue = $this->Oficial->CurrentValue;
			$this->Oficial->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// Oficial
			$CurrentValue = $this->Oficial->CurrentValue;
			$ViewValue = &$this->Oficial->ViewValue;
			$ViewAttrs = &$this->Oficial->ViewAttrs;
			$CellAttrs = &$this->Oficial->CellAttrs;
			$HrefValue = &$this->Oficial->HrefValue;
			$LinkAttrs = &$this->Oficial->LinkAttrs;
			$this->Cell_Rendered($this->Oficial, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
		}
	...
	function SetupFieldCount() {
		...
		if ($this->Oficial->Visible) $this->DtlFldCount += 1;
	}
	...
	function GetExtendedFilter() {
		...
		// Reset search command
			...
			$this->SetSessionFilterValues($this->Establecimiento->SearchValue, $this->Establecimiento->SearchOperator, $this->Establecimiento->SearchCondition, $this->Establecimiento->SearchValue2, $this->Establecimiento->SearchOperator2, 'Establecimiento'); // Field Establecimiento
			...
			if ($this->GetDropDownValue($this->Usuario->DropDownValue, 'Usuario')) {
				$bSetupFilter = TRUE;
			} elseif ($this->Usuario->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_Report1_Usuario'])) {
				$bSetupFilter = TRUE;
			}

			// Field Establecimiento
			if ($this->GetFilterValues($this->Establecimiento)) {
				$bSetupFilter = TRUE;
			}
		...
		if ($bRestoreSession) {
			...
			$this->GetSessionFilterValues($this->Establecimiento); // Field Establecimiento
		}
		...
		$this->BuildExtendedFilter($this->Establecimiento, $sFilter, FALSE, TRUE); // Field Establecimiento
	...
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_Report1_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_Report1_' . $parm);
	...
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_Report1_' . $parm] = $sv1;
		$_SESSION['so_Report1_' . $parm] = $so1;
	...
	function LoadDefaultFilters() {
		...
		*/
		...
		// Field Establecimiento
		$this->SetDefaultExtFilter($this->Establecimiento, "LIKE", NULL, 'AND', "=", NULL);
		if (!$this->SearchCommand) $this->ApplyDefaultExtFilter($this->Establecimiento);
		...
		*/
		...
		// Field Oficial
		// $this->Oficial->DefaultSelectionList = array("val1", "val2");
	...
	function CheckFilter() {
		...
		// Check Establecimiento text filter
		if ($this->TextFilterApplied($this->Establecimiento))
			return TRUE;

		// Check Oficial popup filter
		if (!ewr_MatchedArray($this->Oficial->DefaultSelectionList, $this->Oficial->SelectionList))
			return TRUE;
		return FALSE;
	}
	...
	function ShowFilterList() {
		...
		$sExtWrk = "";
		$sWrk = "";
		$this->BuildExtendedFilter($this->Establecimiento, $sExtWrk);
		$sFilter = "";
		if ($sExtWrk <> "")
			$sFilter .= "<span class=\"ewFilterValue\">$sExtWrk</span>";
		elseif ($sWrk <> "")
		...
		$sExtWrk = "";
		$sWrk = "";
		if (is_array($this->Oficial->SelectionList))
			$sWrk = ewr_JoinArray($this->Oficial->SelectionList, ", ", EWR_DATATYPE_STRING);
		$sFilter = "";
		if ($sExtWrk <> "")
			$sFilter .= "<span class=\"ewFilterValue\">$sExtWrk</span>";
		elseif ($sWrk <> "")
			$sFilter .= "<span class=\"ewFilterValue\">$sWrk</span>";
		if ($sFilter <> "")
			$sFilterList .= "<div><span class=\"ewFilterCaption\">" . $this->Oficial->FldCaption() . "</span>" . $sFilter . "</div>";
		$divstyle = "";
		$divdataclass = "";
	...
	function GetFilterList() {
		...
		// Field Establecimiento
		$sWrk = "";
		if ($this->Establecimiento->SearchValue <> "" || $this->Establecimiento->SearchValue2 <> "") {
			$sWrk = "\"sv_Establecimiento\":\"" . ewr_JsEncode2($this->Establecimiento->SearchValue) . "\"," .
				"\"so_Establecimiento\":\"" . ewr_JsEncode2($this->Establecimiento->SearchOperator) . "\"," .
				"\"sc_Establecimiento\":\"" . ewr_JsEncode2($this->Establecimiento->SearchCondition) . "\"," .
				"\"sv2_Establecimiento\":\"" . ewr_JsEncode2($this->Establecimiento->SearchValue2) . "\"," .
				"\"so2_Establecimiento\":\"" . ewr_JsEncode2($this->Establecimiento->SearchOperator2) . "\"";
		}
		if ($sWrk <> "") {
			if ($sFilterList <> "") $sFilterList .= ",";
			$sFilterList .= $sWrk;
		...
		$sWrk = "";
		if ($sWrk == "") {
			$sWrk = ($this->Oficial->SelectionList <> EWR_INIT_VALUE) ? $this->Oficial->SelectionList : "";
			if (is_array($sWrk))
				$sWrk = implode("||", $sWrk);
			if ($sWrk <> "")
				$sWrk = "\"sel_Oficial\":\"" . ewr_JsEncode2($sWrk) . "\"";
		}
		if ($sWrk <> "") {
			if ($sFilterList <> "") $sFilterList .= ",";
			$sFilterList .= $sWrk;
	...
	function RestoreFilterList() {
		...
		// Field Establecimiento
		$bRestoreFilter = FALSE;
		if (array_key_exists("sv_Establecimiento", $filter) || array_key_exists("so_Establecimiento", $filter) ||
			array_key_exists("sc_Establecimiento", $filter) ||
			array_key_exists("sv2_Establecimiento", $filter) || array_key_exists("so2_Establecimiento", $filter)) {
			$this->SetSessionFilterValues(@$filter["sv_Establecimiento"], @$filter["so_Establecimiento"], @$filter["sc_Establecimiento"], @$filter["sv2_Establecimiento"], @$filter["so2_Establecimiento"], "Establecimiento");
			$bRestoreFilter = TRUE;
		}
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "Establecimiento");
		}

		// Field Oficial
		$bRestoreFilter = FALSE;
		if (array_key_exists("sel_Oficial", $filter)) {
			$sWrk = $filter["sel_Oficial"];
			$sWrk = explode("||", $sWrk);
			$this->Oficial->SelectionList = $sWrk;
			$_SESSION["sel_Report1_Oficial"] = $sWrk;
			$bRestoreFilter = TRUE;
		}
		if (!$bRestoreFilter) { // Clear filter
		}
	...
	function GetPopupFilter() {
		...
		if ($this->DrillDown)
			return "";
			if (is_array($this->Oficial->SelectionList)) {
				$sFilter = ewr_FilterSQL($this->Oficial, "`Oficial`", EWR_DATATYPE_STRING);

				// Call Page Filtering event
				$this->Page_Filtering($this->Oficial, $sFilter, "popup");
				$this->Oficial->CurrentFilter = $sFilter;
	...
	function GetSort() {
		...
		// Set up default sort
		if ($this->getOrderBy() == "") {
			$this->setOrderBy("`Establecimiento` ASC");
			$this->Establecimiento->setSort("ASC");
		}
...
// Form object
var fReport1summary = new ewr_Form("fReport1summary");
...
fReport1summary.Form_CustomValidate = 
...
// Use Ajax
fReport1summary.Lists["sv_Usuario"] = {"LinkField":"sv_Usuario","Ajax":true,"DisplayFields":["sv_Usuario","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
</script>
<?php } ?>
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<script type="text/javascript">
...
<div id="fReport1summary_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<?php
$sSqlWrk = "SELECT DISTINCT `Usuario`, `Usuario` AS `DispFld` FROM `reporte1view`";
$sWhereWrk = "";
...
<div id="c_Establecimiento" class="ewCell form-group">
	...
	<span class="ewSearchOperator"><?php echo $ReportLanguage->Phrase("LIKE"); ?><input type="hidden" name="so_Establecimiento" id="so_Establecimiento" value="LIKE"></span>
	<span class="control-group ewSearchField">
<?php ewr_PrependClass($Page->Establecimiento->EditAttrs["class"], "form-control"); // PR8 ?>
<input type="text" id="sv_Establecimiento" name="sv_Establecimiento" size="30" maxlength="20" placeholder="<?php echo $Page->Establecimiento->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->Establecimiento->SearchValue) ?>"<?php echo $Page->Establecimiento->EditAttributes() ?>>
</span>
</div>
</div>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Usuario"><div class="Report1_Usuario"><span class="ewTableHeaderCaption"><?php echo $Page->Usuario->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="Usuario">
<?php if ($Page->SortUrl($Page->Usuario) == "") { ?>
		<div class="ewTableHeaderBtn Report1_Usuario">
			<span class="ewTableHeaderCaption"><?php echo $Page->Usuario->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Report1_Usuario" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Usuario) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Usuario->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Nombre"><div class="Report1_Nombre"><span class="ewTableHeaderCaption"><?php echo $Page->Nombre->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="Nombre">
<?php if ($Page->SortUrl($Page->Nombre) == "") { ?>
		<div class="ewTableHeaderBtn Report1_Nombre">
			<span class="ewTableHeaderCaption"><?php echo $Page->Nombre->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Report1_Nombre" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Nombre) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Nombre->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Establecimiento"><div class="Report1_Establecimiento"><span class="ewTableHeaderCaption"><?php echo $Page->Establecimiento->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="Establecimiento">
<?php if ($Page->SortUrl($Page->Establecimiento) == "") { ?>
		<div class="ewTableHeaderBtn Report1_Establecimiento">
			<span class="ewTableHeaderCaption"><?php echo $Page->Establecimiento->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Report1_Establecimiento" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Establecimiento) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Establecimiento->FldCaption() ?></span>
...
<?php } ?>
<?php } ?>
<?php if ($Page->Oficial->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Oficial"><div class="Report1_Oficial"><span class="ewTableHeaderCaption"><?php echo $Page->Oficial->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="Oficial">
<?php if ($Page->SortUrl($Page->Oficial) == "") { ?>
		<div class="ewTableHeaderBtn Report1_Oficial">
			<span class="ewTableHeaderCaption"><?php echo $Page->Oficial->FldCaption() ?></span>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, 'Report1_Oficial', false, '<?php echo $Page->Oficial->RangeFrom; ?>', '<?php echo $Page->Oficial->RangeTo; ?>');" id="x_Oficial<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Report1_Oficial" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Oficial) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Oficial->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->Oficial->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->Oficial->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, 'Report1_Oficial', false, '<?php echo $Page->Oficial->RangeFrom; ?>', '<?php echo $Page->Oficial->RangeTo; ?>');" id="x_Oficial<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
		</div>
<?php } ?>
	</td>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_Report1_Usuario"<?php echo $Page->Usuario->ViewAttributes() ?>><?php echo $Page->Usuario->GroupViewValue ?></span></td>
<?php } ?>
<?php if ($Page->Nombre->Visible) { ?>
		<td data-field="Nombre"<?php echo $Page->Nombre->CellAttributes(); ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_Report1_Nombre"<?php echo $Page->Nombre->ViewAttributes() ?>><?php echo $Page->Nombre->GroupViewValue ?></span></td>
<?php } ?>
<?php if ($Page->Establecimiento->Visible) { ?>
		<td data-field="Establecimiento"<?php echo $Page->Establecimiento->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->RecCount ?>_Report1_Establecimiento"<?php echo $Page->Establecimiento->ViewAttributes() ?>><?php echo $Page->Establecimiento->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->Oficial->Visible) { ?>
		<td data-field="Oficial"<?php echo $Page->Oficial->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->RecCount ?>_Report1_Oficial"<?php echo $Page->Oficial->ViewAttributes() ?>><?php echo $Page->Oficial->ListViewValue() ?></span></td>
<?php } ?>
	</tr>
<?php
