...
function ewr_AutoLoad($class) {
	...
	if (substr($class, 0, 2) == "cr") {
		if ($class == "crproyecto_db") {
			$file = "proyectodb2.php";
		} else {
		$fn = "%cls%info.php";
...
function CurrentProjectID() {
	...
	return "{DBB651B2-15BD-4402-ACB5-53B44A927C69}";
}
	...
	function ValidateUser($usr, $pwd, $autologin) {
		...
		if (!$ValidateUser) {
			...
			if ($rs = $conn->Execute($sSql)) {
				if (!$rs->EOF) {
					$ValidateUser = $CustomValidateUser || ewr_ComparePassword($rs->fields('password'), $pwd);
					if ($ValidateUser) {
						$_SESSION[EWR_SESSION_STATUS] = "login";
